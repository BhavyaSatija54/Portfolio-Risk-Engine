...
class TestBasel:
    def test_returns_valid_zone_cdf(self, perfect_var):
        zone = VaRBacktest(*perfect_var).basel_zone_cdf()
        assert zone in ("green", "yellow", "red")

    def test_returns_valid_zone_250(self, perfect_var):
        zone = VaRBacktest(*perfect_var).basel_zone_250()
        assert zone in ("green", "yellow", "red")

    def test_zone_via_run(self, perfect_var):
        m = VaRBacktest(*perfect_var).run()
        assert m.basel_zone in ("green", "yellow", "red")
        assert m.tl_colour_hex.startswith("#")
